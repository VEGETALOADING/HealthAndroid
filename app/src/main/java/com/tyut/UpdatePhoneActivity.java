        ...
        switch (v.getId()){
            ...
            case R.id.getvalcode_btn:
                ...
                if(phoneNum.length() != 11){
                    ...
                    OkHttpUtils.get("http://"+this.getString(R.string.localhost)+"/portal/user/isbound.do?phone="+phoneNum,
                            new OkHttpCallback(){
                                @Override
                                public void onFinish(String status, String msg) {
                                    ...
                                    if(serverResponse.getStatus() != 0){
                                        ...
                                        OkHttpUtils.get("http://"+UpdatePhoneActivity.this.getString(R.string.localhost)+"/portal/user/sendvalcode.do?phone="+phoneNum+"&valcode="+generateValcode,
                                                new OkHttpCallback(){
                                                    @Override
                                                    public void onFinish(String status, String msg) {
                                                        super.onFinish(status, msg);
            ...
            case R.id.bount_btn:
                ...
                if(inputValcode.equals(generateValcode)){
                    ...
                    //验证码正确，绑定手机号
                    OkHttpUtils.get("http://"+this.getString(R.string.localhost)+"/portal/user/update.do?id="+userVO.getId()+"&phone="+phoneNum,
                            new OkHttpCallback(){
                                @Override
                                public void onFinish(String status, String msg) {
                                    super.onFinish(status, msg);
