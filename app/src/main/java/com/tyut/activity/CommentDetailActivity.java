...
public class CommentDetailActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    private void getCommentVO(){
        OkHttpUtils.get("http://"+getString(R.string.url)+":8080/portal/comment/find.do?id="+commentVO.getId()+"&currentUserId="+userVO.getId(),
                new OkHttpCallback(){
                    @Override
                    public void onFinish(String status, String msg) {
                        super.onFinish(status, msg);
                        ...
                        commentVO = serverResponse.getData().get(0);
    ...
    private void updateReplyView(){
        if(commentVO.getReplyList().size() != 0) {
            replyCount_tv.setText(commentVO.getReplyList().size()+"");
