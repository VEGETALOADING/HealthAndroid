...
public class GirthRulerView extends View {

    private Paint paint;

    private Context context;

    private int maxValue = 1300;
    /**
     * 起点x的坐标
     */
...
//    private float unit = 10f;
    ...
    public GirthRulerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
...
//            String text = format.format(i + 1) + "";//从0开始计数时不用加1
            String text = format.format(i * iValue+20) + "";//乘以每刻度的值iValue
            // 获取文本的宽度
...
//        final int scrollX = (int) ((scaleValue - 1.0f) * gap * unit);//从0开始计数时不用减去1
        final int scrollX = (int) (scaleValue * gap * unit);//每个值在设置刻度时会乘以10，所以除去
